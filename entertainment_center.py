...
api_key = "e636464b21df7ea6ab6b26b273fea8bc"
# Make a get request to get the list of latest movies from the Movie Db api.
response = requests.get(
    "https://api.themoviedb.org/4/list/33436?api_key="+api_key)
...
# Loop through each response results to get all the list of
# movies and add it in a list according to our Movie model class.
for db_movie in response_data["results"]:
    ...
    movie_id = str(db_movie["id"])
    movie_base_url = "https://api.themoviedb.org/3/movie/"
    base_url_params = "/videos?api_key="+api_key

    # get youtube key from a different api of movie db
    # and then create a final complete youtube url
    trailer_url = movie_base_url+movie_id+base_url_params+"&language=en-US"
